...
function initDemoHandlers(): void {
  ...
  on(document.getElementById('demo-toggleclass-btn')!, 'click', () => {
    ...
    html(
      cssOutput as HTMLElement,
      `toggleClass('snp-highlighted'): class <strong>${has ? 'added' : 'removed'}</strong>`,
    );
    html(
      cssCode as HTMLElement,
      createDemoCodeBlock(
  ...
  on(document.getElementById('demo-hide-btn')!, 'click', () => {
    hide(visBox as any);
    html(
      visOutput as HTMLElement,
      `${ICONS.eyeOff} hide() - element hidden (stores current display, sets display:none)`,
    );
    html(
      visCode as HTMLElement,
      createDemoCodeBlock(
  ...
  on(document.getElementById('demo-add-before-btn')!, 'click', () => {
    ...
    html(
      domOutput as HTMLElement,
      `${ICONS.check} before(baseElement, newEl) - inserted BEFORE Base Element (inside same container)`,
    );
    html(
      domCode as HTMLElement,
      createDemoCodeBlock(
  ...
  on(document.getElementById('demo-add-after-btn')!, 'click', () => {
    ...
    html(
      domOutput as HTMLElement,
      `${ICONS.check} after(baseElement, newEl) - inserted AFTER Base Element (inside same container)`,
    );
    html(
      domCode as HTMLElement,
      createDemoCodeBlock(
  ...
  on(document.getElementById('demo-get-before-btn')!, 'click', () => {
    ...
    html(
      domOutput as HTMLElement,
      `${ICONS.info} before(baseElement) - returned: <strong>${prevEl ? prevEl.textContent : 'null (no previous sibling)'}</strong>`,
    );
    html(
      domCode as HTMLElement,
      createDemoCodeBlock(
  ...
  on(document.getElementById('demo-get-after-btn')!, 'click', () => {
    ...
    html(
      domOutput as HTMLElement,
      `${ICONS.info} after(baseElement) - returned: <strong>${nextEl ? nextEl.textContent : 'null (no next sibling)'}</strong>`,
    );
    html(
      domCode as HTMLElement,
      createDemoCodeBlock(
  ...
  on(document.getElementById('demo-attr-get-btn')!, 'click', () => {
    ...
    html(
      attrOutput as HTMLElement,
      `${ICONS.info} attr(el, 'data-custom'): <strong>${val}</strong>, hasClass('snp-highlighted'): <strong>${has}</strong>`,
    );
    html(
      attrCode as HTMLElement,
      createDemoCodeBlock(
  ...
  on(document.getElementById('demo-data-btn')!, 'click', () => {
    ...
    html(
      attrOutput as HTMLElement,
      `${ICONS.database} dataByPrefix(el, 'pref'): <strong>${JSON.stringify(prefixed)}</strong>`,
    );
    html(
      attrCode as HTMLElement,
      createDemoCodeBlock(
  ...
  on(document.getElementById('demo-val-btn')!, 'click', () => {
    ...
    html(
      valOutput as HTMLElement,
      `${ICONS.check} val(input, 'Value has been set!') - value set, returns element (chainable)`,
    );
    html(
      valCode as HTMLElement,
      createDemoCodeBlock(
  ...
  on(document.getElementById('demo-tohtml-btn')!, 'click', () => {
    ...
    html(
      htmlOutput as HTMLElement,
      `${ICONS.code} toHtml(el): <strong>${escapeHtml(result)}</strong> (uses outerHTML)`,
    );
    html(
      htmlCodeEl as HTMLElement,
      createDemoCodeBlock(
  ...
  on(document.getElementById('demo-appendtext-btn')!, 'click', () => {
    ...
    html(
      htmlOutput as HTMLElement,
      `${ICONS.check} appendText(container, ' [text node] ') - Text node created and appended`,
    );
    html(
      htmlCodeEl as HTMLElement,
      createDemoCodeBlock(
  ...
  on(document.getElementById('demo-merge-text-btn')!, 'click', () => {
    ...
    html(
      htmlOutput as HTMLElement,
      `${ICONS.check} mergeAdjacentTextNodes(): ${beforeCount} nodes → <strong>${afterCount}</strong> nodes (merged)`,
    );
    html(
      htmlCodeEl as HTMLElement,
      createDemoCodeBlock(
  ...
  on(document.getElementById('demo-replacechildren-btn')!, 'click', () => {
    ...
    if (parent && wrapper.children.length > 0) {
      ...
      html(
        htmlOutput as HTMLElement,
        `${ICONS.check} replaceWithChildren(wrapper) - wrapper removed, children remain as direct children of parent`,
      );
    } else {
      html(
        htmlOutput as HTMLElement,
        `${ICONS.info} replaceWithChildren() - no children to preserve or wrapper already removed`,
      );
    }
    html(
      htmlCodeEl as HTMLElement,
  ...
  on(document.getElementById('demo-query-btn')!, 'click', () => {
    let c = 0;
    query('.query-target', (el, i) => {
      c++;
    });
    html(
      queryOutput as HTMLElement,
      `${ICONS.hash} query('.query-target', callback): iterated <strong>${c}</strong> elements`,
    );
    html(
      queryCode as HTMLElement,
      createDemoCodeBlock(
  ...
  on(document.getElementById('demo-querylist-btn')!, 'click', () => {
    ...
    html(
      queryOutput as HTMLElement,
      `${ICONS.hash} queryList('.query-target'): <strong>${items.length}</strong> items returned as array`,
    );
    html(
      queryCode as HTMLElement,
      createDemoCodeBlock(
  ...
  on(document.getElementById('demo-closest-btn')!, 'click', () => {
    ...
    html(
      queryOutput as HTMLElement,
      `${ICONS.link} closest(target, container): <strong>${found ? 'found! (returns the parent)' : 'not found (returns false)'}</strong>`,
    );
    html(
      queryCode as HTMLElement,
      createDemoCodeBlock(
  ...
  on(document.getElementById('demo-getchildnodes-btn')!, 'click', () => {
    ...
    html(
      queryOutput as HTMLElement,
      `${ICONS.box} getChildNodes(container): <strong>${nodes.length}</strong> child nodes (including text nodes)`,
    );
    html(
      queryCode as HTMLElement,
      createDemoCodeBlock(
  ...
  on(document.getElementById('demo-offset-btn')!, 'click', () => {
    ...
    html(
      ...
      `${ICONS.box} offset(el): { top: <strong>${off.top}px</strong>, left: <strong>${off.left}px</strong> } - relative to document`,
    );
    html(
  ...
  on(document.getElementById('demo-outersize-btn')!, 'click', () => {
    ...
    html(
      measureOutput as HTMLElement,
      `${ICONS.box} outerSize(el): { width: <strong>${sz.width}px</strong>, height: <strong>${sz.height}px</strong> } - includes margin`,
    );
    html(
      measureCode as HTMLElement,
      createDemoCodeBlock(
  ...
  on(document.getElementById('demo-off-btn')!, 'click', () => {
    ...
    html(
      eventsOutput as HTMLElement,
      `${ICONS.x} off('click.demoOn') - handler removed. Click "on() click" - nothing will happen`,
    );
    html(
      eventsCode as HTMLElement,
      createDemoCodeBlock(
  ...
  on(document.getElementById('demo-rebind-btn')!, 'click', () => {
    ...
    html(
      eventsOutput as HTMLElement,
      `${ICONS.refresh} rebind('click.demoOn', newHandler) - old handler replaced with new one`,
    );
    html(
      eventsCode as HTMLElement,
      createDemoCodeBlock(
  ...
  on(document.getElementById('demo-isempty-btn')!, 'click', () => {
    ...
    html(
      stringCheckCode as HTMLElement,
      createDemoCodeBlock(
...

{
  ...
  test.assertEquals(
    el.style.color,
    'red',
    'css() sets single property (string)',
    `css(el, 'color', 'red');\n// Sets color to red`,
    `css(el, 'color', 'red');\n// Sets color to red`,
  );
  ...
  test.assertEquals(
    el.style.height,
    '100px',
    'css() sets multiple properties via object',
  ...
  test.assertEquals(
    color,
    'red',
    'css() returns computed style value (getter)',
  ...
  test.assert(
    !el.style.color,
    'css() removes property with empty string',
    `css(el, 'color', '');\n// Empty string triggers removeProperty()`,
    `css(el, 'color', '');\n// Empty string triggers removeProperty()`,
  );
  ...
  test.assert(
    !el.style.width,
    'css() removes property with null value',
  ...
  test.assertEquals(
    ...
    'green',
    'css() accepts kebab-case property names',
...

{
  ...
  test.assert(
    hasClass(el, 'test-class'),
    'addClass() adds a single class',
  ...
  test.assert(
    hasClass(el, 'class1') && hasClass(el, 'class2'),
    'addClass() adds multiple classes via space-separated string',
  ...
  test.assert(
    hasClass(el, 'class3') && hasClass(el, 'class4'),
    'addClass() adds multiple classes via array',
    `addClass(el, ['class3', 'class4']);`,
    `addClass(el, ['class3', 'class4']);`,
  );
  ...
  test.assert(
    !hasClass(el, 'class1'),
    'removeClass() removes a single class',
    `removeClass(el, 'class1');`,
    `removeClass(el, 'class1');`,
  );
  ...
  test.assert(
    !hasClass(el, 'class2') && !hasClass(el, 'class3'),
    'removeClass() removes multiple via string',
    `removeClass(el, 'class2 class3');`,
    `removeClass(el, 'class2 class3');`,
  );
  ...
  test.assert(
    hasClass(el, 'toggle-me'),
    'toggleClass() adds class when absent',
  ...
  test.assert(
    !hasClass(el, 'toggle-me'),
    'toggleClass() removes class when present',
  ...
  toggleClass(el, ['a', 'b']);
  test.assert(
    hasClass(el, 'a') && hasClass(el, 'b'),
    'toggleClass() toggles multiple via array',
  ...
  test.assert(
    !hasClass(null, 'any'),
    'hasClass() returns false for null element',
...

{
  ...
  test.assertEquals(
    ...
    'div',
    'make() creates element with correct tag',
  ...
  test.assertEquals(
    div.id,
    'test-div',
    'make() callback receives created element',
    `make('div', (el) => {\n  el.id = 'test-div';\n  el.textContent = 'Hello';\n});`,
    `make('div', (el) => {\n  el.id = 'test-div';\n  el.textContent = 'Hello';\n});`,
  );
...

{
  ...
  test.assertEquals(
    count,
    3,
    'query() calls callback for each element',
  ...
  test.assertEquals(
    items.length,
    3,
    'queryList() returns array of elements',
    `const items: HTMLElement[] = queryList<HTMLElement>('.query-item');\nconsole.log(items.length); // 3`,
    `const items = queryList('.query-item');\nconsole.log(items.length); // 3`,
  );

  test.assertEquals(
    queryLength('.query-item'),
    3,
    'queryLength() returns element count',
  ...
  test.assertEquals(
    queryLength('.nested', nested),
    1,
    'query() supports context parameter',
...

{
  ...
  test.assertEquals(
    html(el),
    '<span>Test</span>',
    'html() with null does not change content',
    `html(el, null);\n// Content unchanged`,
    `html(el, null);\n// Content unchanged`,
  );
  ...
  test.assertEquals(
    toHtml(span),
    '<span>Hello</span>',
    'toHtml() converts element to HTML string',
    `toHtml(spanElement);\n// Uses outerHTML for elements`,
    `toHtml(spanElement);\n// Uses outerHTML for elements`,
  );

  test.assertEquals(
    toHtml([make('div'), make('span')]),
    '<div></div><span></span>',
    'toHtml() converts array to HTML',
...

{
  ...
  const child1 = make('span');
  child1.textContent = 'first';
  const child2 = make('span');
  ...
  const child0 = make('span');
  child0.textContent = 'zero';
  ...
  test.assertEquals(
    ...
    beforeEl,
    'before() returns previous sibling when no second argument',
    `const prevSibling = before(child1);\n// Returns the element immediately before child1`,
    `const prevSibling = before(child1);\n// Returns the element immediately before child1`,
  );
  test.assertEquals(
    before(child0 as HTMLElement),
    null,
    'before() returns null when no previous sibling exists',
    `before(child0); // null (no previous sibling)`,
    `before(child0); // null (no previous sibling)`,
  );

  const afterEl = make('span');
  afterEl.textContent = 'after';
  after(child1, afterEl);
  test.assertEquals(
    child1.nextSibling,
    afterEl,
    'after() inserts after target (with second arg)',
  ...
  test.assertEquals(
    ...
    afterEl,
    'after() returns next sibling when no second argument',
    `const nextSibling = after(child1);\n// Returns the element immediately after child1`,
    `const nextSibling = after(child1);\n// Returns the element immediately after child1`,
  );
  test.assertEquals(
    after(child2 as HTMLElement),
    null,
    'after() returns null when no next sibling exists',
...

{
  ...
  test.assertEquals(
    attr(el, 'data-x'),
    '1',
    'attr() sets multiple attributes via object',
    `attr(el, { 'data-x': '1', 'data-y': '2' });`,
    `attr(el, { 'data-x': '1', 'data-y': '2' });`,
  );
  test.assertEquals(
    attr(el, 'data-y'),
    '2',
    'attr() sets multiple attributes via object (2)',
  ...
  test.assert(
    ...
    'removeAttr() removes single attribute',
    `removeAttr(el, 'data-x');`,
    `removeAttr(el, 'data-x');`,
  );
  ...
  test.assert(
    !el.hasAttribute('data-y') && !el.hasAttribute('data-test'),
    'removeAttr() removes multiple attributes',
    `removeAttr(el, ['data-y', 'data-test']);`,
    `removeAttr(el, ['data-y', 'data-test']);`,
  );
...

{
  ...
  test.assertEquals(
    el.dataset.userId,
    '123',
    'data() sets single data attribute',
  ...
  test.assertEquals(
    el.dataset.role,
    'admin',
    'data() sets multiple data attributes',
    `data(el, { role: 'admin', score: '100' });`,
    `data(el, { role: 'admin', score: '100' });`,
  );
  ...
  test.assertDeepEqual(
    prefixed,
    { xx: 33, ok: 'ok' },
    'dataByPrefix() collects prefixed data attrs',
...

{
  ...
  test.assert(
    closest(grandchild, parent) === parent,
  ...
  test.assert(
    closest(grandchild, child) === child,
    'closest() finds direct parent',
    `// Works with any ancestor level`,
    `// Works with any ancestor level`,
  );

  test.assert(
    !closest(grandchild, make('div')),
    'closest() returns false if not found',
  ...
  test.assert(
    !closest(null, parent),
    'closest() returns false for null child',
...

{
  ...
  test.assertEquals(
    val(emptyInput),
    '',
    'val() returns empty string for empty input',
    `val(emptyInput); // ''`,
    `val(emptyInput); // ''`,
  );
...

{
  ...
  test.assertEquals(
    el.textContent,
    'Hello World',
    'appendText() adds text to element',
...

{
  ...
  test.assert(
    ...
    'replaceWithChildren() removes wrapper',
    `replaceWithChildren(wrapperElement);\n// Wrapper removed, children remain`,
    `replaceWithChildren(wrapperElement);\n// Wrapper removed, children remain`,
  );
  test.assert(
    parent.contains(child1) && parent.contains(child2),
    'replaceWithChildren() keeps children',
...

{
  ...
  test.assert(
    typeof elOffset.top === 'number' && typeof elOffset.left === 'number',
    'offset() returns coordinates',
...

{
  ...
  test.assertEquals(
    nodes.length,
    2,
    'getChildNodes() returns child nodes array',
...

{
  ...
  test.assertEquals(
    clicks,
    2,
    'on() handler fires multiple times',
  ...
  test.assertEquals(
    clicks,
    2,
    'off() removes handler by event.id',
  ...
  test.assertEquals(
    clicks,
    3,
    'off() without id removes all handlers for event type',
    `off(button, 'mouseenter');\n// Removes ALL mouseenter handlers`,
    `off(button, 'mouseenter');\n// Removes ALL mouseenter handlers`,
  );

  let rebindCount = 0;
  on(button, 'click.rebind', () => {
    rebindCount = 1;
  });
  button.click();
  test.assertEquals(rebindCount, 1, 'on() before rebind');
  rebind(button, 'click.rebind', () => {
    rebindCount = 2;
  });
  button.click();
  test.assertEquals(
...

{
  ...
  test.assertEquals(
    ...
    '<p>Content</p>',
    'stripFragment() removes fragment comments',
    `stripFragment('<!--StartFragment--><p>Content</p><!--EndFragment-->');`,
    `stripFragment('<!--StartFragment--><p>Content</p><!--EndFragment-->');`,
  );
}
...

{
  ...
  test.assert(
    nodes.length >= 2,
    'parseHtml() parses HTML string into nodes',
  ...
  test.assert(
    fragNodes.length > 0,
    'parseHtml() strips fragment with isStripFragment=true',
...

{
  test.assert(
    isEmptyString('   '),
    'isEmptyString(): true for whitespace',
    `isEmptyString('   '); // true`,
    `isEmptyString('   '); // true`,
  );
  test.assert(
    isEmptyString(''),
    'isEmptyString(): true for empty',
    `isEmptyString(''); // true`,
    `isEmptyString(''); // true`,
  );
  test.assert(
    isEmptyString(null),
    'isEmptyString(): true for null',
    `isEmptyString(null); // true`,
    `isEmptyString(null); // true`,
  );
  test.assert(
    !isEmptyString('text'),
    'isEmptyString(): false for non-empty',
  ...
  test.assert(
    isNullOrEmpty(''),
    'isNullOrEmpty(): true for empty string',
    `isNullOrEmpty(''); // true`,
    `isNullOrEmpty(''); // true`,
  );
  test.assert(
    isNullOrEmpty(null),
    'isNullOrEmpty(): true for null',
    `isNullOrEmpty(null); // true`,
    `isNullOrEmpty(null); // true`,
  );
  test.assert(
    !isNullOrEmpty('x'),
    'isNullOrEmpty(): false for non-empty',
  ...
  test.assert(
    isWhitespace('   '),
    'isWhitespace(): true for spaces',
    `isWhitespace('   '); // true (non-empty, only whitespace)`,
    `isWhitespace('   '); // true (non-empty, only whitespace)`,
  );
  test.assert(
    !isWhitespace('  a  '),
    'isWhitespace(): false with chars',
...

{
  ...
  test.assertEquals(
    upperFirst(''),
    '',
    'upperFirst() handles empty string',
    `upperFirst(''); // ''`,
    `upperFirst(''); // ''`,
  );
  ...
  test.assertEquals(
    lowerFirst(''),
    '',
    'lowerFirst() handles empty string',
    `lowerFirst(''); // ''`,
    `lowerFirst(''); // ''`,
  );
...

{
  ...
  test.assertEquals(
    truncate('Short', 10),
    'Short',
    'truncate() does not truncate short strings',
    `truncate('Short', 10);\n// 'Short' (no change)`,
    `truncate('Short', 10);\n// 'Short' (no change)`,
  );
}
...

{
  ...
  test.assertEquals(reverse('a'), 'a', 'reverse(): single char', `reverse('a'); // 'a'`, `reverse('a'); // 'a'`);
  ...
  test.assert(
    formatBytes(1024, 2, 'en').includes('KB'),
    'formatBytes(): 1024 => KB',
    `formatBytes(1024);\n// '1 KB' (localized)`,
    `formatBytes(1024);\n// '1 KB' (localized)`,
  );
  test.assert(
    formatBytes(1048576, 2, 'en').includes('MB'),
    'formatBytes(): 1048576 => MB',
    `formatBytes(1048576);\n// '1 MB' (localized)`,
    `formatBytes(1048576);\n// '1 MB' (localized)`,
  );
  test.assert(
    formatBytes(1073741824, 2, 'en').includes('GB'),
    'formatBytes(): 1073741824 => GB',
...

{
  ...
  test.assertEquals(
    r1.length,
    8,
    'randString(): correct length (alphanumeric)',
  ...
  test.assertEquals(
    pluralize(1, ['год', 'года', 'лет']),
    '1 год',
    'pluralize(): singular (1)',
    `pluralize(1, ['год', 'года', 'лет']); // '1 год'`,
    `pluralize(1, ['год', 'года', 'лет']); // '1 год'`,
  );
  test.assertEquals(
    pluralize(3, ['год', 'года', 'лет']),
    '3 года',
    'pluralize(): few (3)',
  ...
  test.assertEquals(
    pluralize(11, ['год', 'года', 'лет']),
    '11 лет',
    'pluralize(): exception (11)',
    `pluralize(11, ['год', 'года', 'лет']); // '11 лет'`,
    `pluralize(11, ['год', 'года', 'лет']); // '11 лет'`,
  );
  test.assertEquals(
    pluralize(21, ['год', 'года', 'лет']),
    '21 год',
    'pluralize(): compound (21)',
    `pluralize(21, ['год', 'года', 'лет']); // '21 год'`,
    `pluralize(21, ['год', 'года', 'лет']); // '21 год'`,
  );
  test.assertEquals(
    pluralize(2, ['яблоко', 'яблока', 'яблок'], false),
    'яблока',
    'pluralize(): hide number',
